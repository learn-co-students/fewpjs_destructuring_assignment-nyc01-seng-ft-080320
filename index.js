...
const [moo,Nehh,baww,oink,cluck] = farmAnimals.split(' ')

// 2. Bolt the horse wandered off, so just give us four animals, and let's name them Bessie, Dolly, Babe, and Little.
 const [Bessie, Dolly, Babe, Little] = farmAnimals.split(" ").filter(word => word != 'horse')
...
const [black_and_white, black, pink] = farmAnimals.split(" ").filter((word)=>{
  if (word != 'horse' || word != 'chicken'){
    return word
  }
})
...
const [r, o, y, g, b, v] = colors.filter(word => word != 'indigo')

// 6. But wait! Indigo is now feeling *super* left out. Let's only assign indigo using indg. 
const [indg] = colors.filter(word => word == 'indigo')
